...
Features:
- Automatic detection of Parts, Chapters, and Articles
- Hierarchical folder organization
- Individual JSON files for each Article with complete content
- Metadata extraction including page ranges, equations, figures
- Cross-referencing between volumes
...
class MaxwellTOCExtractor:
    """Extracts and organizes Maxwell's treatise into hierarchical structure"""
    ...
    def create_hierarchical_structure(self, hierarchy: Dict[str, Any], volume_num: int):
        """
        Create the hierarchical folder/file structure

        Args:
            hierarchy: Hierarchical structure dictionary
            volume_num: Volume number
        """
        volume_dir = self.base_output_dir / f"Volume_{volume_num}"
        ...
        for part in hierarchy["volume"]["parts"]:
            part_dir = volume_dir / f"Part_{part['part_number']:02d}_{self._sanitize_filename(part['title'])}"
            ...
            for chapter in part["chapters"]:
                chapter_dir = part_dir / f"Chapter_{chapter['chapter_number']:03d}_{self._sanitize_filename(chapter['title'])}"
                ...
                for article in chapter["articles"]:
                    article_filename = f"Article_{article['article_number']:04d}_{self._sanitize_filename(article['title'])}.json"
    ...
    def generate_master_index(self):
        ...
        master_index = {
            "maxwell_toc_index": {
                "created_at": datetime.now().isoformat(),
                "total_volumes": len([d for d in self.base_output_dir.iterdir() if d.is_dir() and d.name.startswith('Volume_')]),
                "volumes": {},
                "navigation": {
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Maxwell TOC Extractor & Hierarchical Organizer")
