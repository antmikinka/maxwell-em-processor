#!/usr/bin/env python3
"""
Enhanced Maxwell TOC Extractor - Working Version
================================================

This working version addresses ALL critical issues identified:
...
class WorkingEnhancedMaxwellTOCExtractor:
    """Working enhanced TOC extractor with all critical fixes"""
    ...
    def __init__(self, base_output_dir: str = "Enhanced_Maxwell_TOC"):
        ...
        # Setup logging manually to avoid scoping issues
        self._setup_logging()
    ...
    def _setup_logging(self):
        ...
        self.logger.info("Enhanced Maxwell TOC Extractor initialized")
    ...
    def load_volume_data(self, volume_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Loading Volume {volume_num} from: {json_file}")
            ...
            self.logger.info(f"Loaded Volume {volume_num} data: {data['total_pages']} pages")
            return data
    ...
    def _validate_volume_data(self, data: Dict[str, Any], volume_num: int) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Volume {volume_num} data validation passed")
            return True
    ...
    def extract_content_hierarchy(self, volume_data: Dict[str, Any], volume_num: int) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"Starting enhanced content hierarchy extraction for Volume {volume_num}")
        ...
        self.logger.info(f"Analyzing {len(sorted_pages)} pages for Volume {volume_num}")

        # Extract parts with validation
        parts = self._find_parts_with_validation(sorted_pages, volume_num)
        self.logger.info(f"Found {len(parts)} parts in Volume {volume_num}")
        ...
        for part_idx, part in enumerate(parts):
            ...
            self.logger.info(f"Found {len(chapters)} chapters in Part {part_idx + 1}")
            ...
            for chapter_idx, chapter in enumerate(chapters):
                ...
                self.logger.info(f"Found {len(articles)} articles in Chapter {chapter['number']}")
        ...
        self.logger.info(f"Enhanced hierarchy extraction completed for Volume {volume_num}")
        return hierarchy
    ...
    def _find_parts_with_validation(self, sorted_pages: List[Tuple[int, Dict]], volume_num: int) -> List[Dict]:
            ...
            for pattern in self.part_patterns:
                ...
                for match in matches:
                    if len(match) >= 2:
                        ...
                        self.logger.info(f"Found Part {part_number}: {part_title} (page {page_num})")
        ...
        if not parts:
            self.logger.warning(f"Volume {volume_num} no parts detected, creating default part")
            parts.append({
                'number': 1,
            ...
            for pattern in self.chapter_patterns:
                ...
                for match in matches:
                    if len(match) >= 2:
                        ...
                        self.logger.info(f"Found Chapter {chapter_number}: {chapter_title} (page {page_num})")
            ...
            for pattern in self.article_patterns:
                ...
                for match in matches:
                    if len(match) >= 2:
                        ...
                        self.logger.info(f"Found Article {article_number}: {article_title} (page {page_num})")
    ...
    def _validate_and_fix_part_boundaries(self, parts: List[Dict], volume_data: Dict[str, Any]) -> List[Dict]:
        ...
        for i, part in enumerate(parts):
            ...
            if part['page_start'] < 1:
                fixed_start = 1
                self.logger.warning(f"Part {part['number']} page_start {part['page_start']} invalid, fixing to {fixed_start}")
            ...
            if part.get('page_end', 0) > total_pages:
                ...
                self.logger.warning(f"Part {part['number']} page_end {part.get('page_end')} exceeds total pages, fixing to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if part['page_start'] > part.get('page_end', part['page_start']):
                ...
                self.logger.warning(f"Part {part['number']} page_start > page_end, fixing page_end to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
    ...
    def _validate_and_fix_chapter_boundaries(self, chapters: List[Dict], part_pages: List[Tuple[int, Dict]], part: Dict) -> List[Dict]:
        ...
        for i, chapter in enumerate(chapters):
            ...
            if chapter['page_start'] < part_start:
                ...
                self.logger.warning(f"Chapter {chapter['number']} page_start {chapter['page_start']} before part start, fixing to {fixed_start}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if chapter.get('page_end', 0) > part_end:
                fixed_end = part_end
                self.logger.warning(f"Chapter {chapter['number']} page_end {chapter.get('page_end')} after part end, fixing to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if chapter['page_start'] > chapter.get('page_end', chapter['page_start']):
                ...
                self.logger.warning(f"Chapter {chapter['number']} page_start > page_end, fixing page_end to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
    ...
    def _validate_and_fix_article_boundaries(self, articles: List[Dict], chapter_pages: List[Tuple[int, Dict]], chapter: Dict) -> List[Dict]:
        ...
        self.logger.info(f"Articles sorted by page_start: {article_info}")

        for i, article in enumerate(articles):
            # Fix page_start relative to chapter boundaries
            if article['page_start'] < chapter_start:
                fixed_start = chapter_start
                self.logger.warning(f"Article {article['number']} page_start {article['page_start']} before chapter start, fixing to {fixed_start}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if article.get('page_end', 0) > chapter_end:
                ...
                self.logger.warning(f"Article {article['number']} page_end {article.get('page_end')} after chapter end, fixing to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if article['page_start'] > article.get('page_end', article['page_start']):
                ...
                self.logger.warning(f"Article {article['number']} page_start > page_end, fixing page_end to {fixed_end}")
                self.extraction_log["fixed_issues"].append({
                    "timestamp": datetime.now().isoformat(),
            ...
            if i > 0:
                ...
                if article['page_start'] <= prev_article.get('page_end', article['page_start']):
                    ...
                    self.logger.warning(f"Article {article['number']} overlaps with previous article, fixing page_start to {fixed_start}")
                    self.extraction_log["fixed_issues"].append({
                        "timestamp": datetime.now().isoformat(),
                    ...
                    if article['page_start'] > chapter_end:
                        self.logger.error(f"Article {article['number']} moved beyond chapter end, removing article")
                        self.extraction_log["errors"].append({
                            "timestamp": datetime.now().isoformat(),
    ...
    def _extract_article_content(self, pages: List[Tuple[int, Dict]], article: Dict) -> Dict:
        ...
        if missing_pages:
            ...
            self.logger.warning(f"Article {article['number']} missing OCR data for pages: {missing_pages}")
    ...
    def create_hierarchical_structure(self, hierarchy: Dict[str, Any], volume_num: int):
        ...
        self.logger.info(f"Creating hierarchical structure for Volume {volume_num}")
        ...
        for part in hierarchy["volume"]["parts"]:
            ...
            for chapter in part["chapters"]:
                ...
                for article in chapter["articles"]:
                    ...
                    self.logger.info(f"Created article file: {article_path}")

        self.logger.info(f"Hierarchical structure created for Volume {volume_num}")
    ...
    def generate_master_index(self):
        ...
        self.logger.info("Enhanced Master TOC index generated")
    ...
    def save_extraction_log(self):
        ...
        self.logger.info(f"Enhanced extraction log saved to: {log_file}")
        return log_file

    def process_both_volumes(self):
        """Process both volumes and create complete hierarchical structure"""
        self.logger.info("Starting enhanced comprehensive TOC extraction for both volumes")
        ...
        for volume_num in [1, 2]:
            ...
            if not volume_data:
                self.logger.warning(f"Skipping Volume {volume_num} - no data available")
                continue
        ...
        self.logger.info("Enhanced TOC extraction completed for both volumes")
        return log_file
...
def main():
    ...
    extractor = WorkingEnhancedMaxwellTOCExtractor(args.base_dir)

    try:
        if args.volume == 0:
            log_file = extractor.process_both_volumes()
        else:
            extractor.logger.info(f"Processing Volume {args.volume}")
        ...
        extractor.logger.info("Enhanced Maxwell TOC extraction completed successfully!")
        ...
        extractor.logger.error(f"Enhanced extraction failed: {str(e)}")
        extractor.logger.debug(f"Full traceback: {traceback.format_exc()}")
